...
//            pages.forEach(pom-> System.out.println(pom.getPageName()));
     ...
     */
    public List<TestCase> load(String path) {
        List<TestCase> testCases = null;
        ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
        TypeReference<List<TestCase>> typeReference = new TypeReference<List<TestCase>>() {
        };
        try {
            testCases = mapper.readValue(BasePage.class.getResourceAsStream(path), typeReference);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return testCases;
    ...
    public void loadPages(String dir) {
        Stream.of(new File(dir).list(
                new FilenameFilter() {
                    @Override
                    public boolean accept(File dir, String name) {
                        return name.contains("Page.yaml");
                    }
                }
        )).forEach(path -> {
            path = dir + "/" + path;
